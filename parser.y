...
%token <value> MAIN INT INTPTR CHAR VOID CHARPTR CHAR_VAL FLOAT FLOAT_VAL FLOATPTR BOOLEAN TRUE FALSE 
...
%type <nPtr> function main expression expressions_ne expressions identifiers next_identifier pdecleration var_types type
%type <nPtr> parameter_list functions func_body statement assignment update lhs if_else_block if_else_statement code_block func_call
%type <nPtr>  if if_else while do_while for statements statements_ne variable_declaration variable_ass
...
start:	main			{printTree(mkNode("CODE",1,$1), 0, 0, 1);printf("\n");}
	|	functions main 	{printTree(mkNode("CODE",2,mkParent("",$1),mkParent("",$2)), 0, 0, 1);printf("\n");}
	|	functions		{printTree(mkNode("CODE",1,mkParent("",$1)), 0, 0, 1);printf("\n");}
	;
							
main:  MAIN '(' ')' ':' VOID '{' func_body '}'
         {$$ = mkNode("MAIN",1,$$);}
		|  MAIN '(' ')' ':' INT '{' func_body '}'
         {$$ = mkNode("MAIN",1,$$);}
		 ;
...
function:	var_start var_types ID '(' parameter_list ')' ':' var_static '{' func_body '}'			
		...
		|	var_start var_types ID '(' ')' ':' var_static '{' func_body '}'
...
assignment:	lhs '=' expression 		{$$ = mkNode("ASS",2,mkNode("",1,$1),$3);}
		|	lhs '=' STRING_VAL 		{$$ = mkNode("ASS",2,$1,mkNode($3,0));}
		;
...
func_call:	lhs '=' ID '(' expressions ')' ';'	{$$ = mkNode("FUNCTION_CALL",1,(mkNode("ASS",3,$1,mkNode($3,0),mkParent("ARGS",$5))));}
		|	ID '(' expressions ')' ';'			{$$ = mkNode("FUNCTION_CALL",1,mkParent($1,mkParent("ARGS",$3)));}
		|	ID '('')' ';'						{$$ = mkNode("FUNCTION_CALL",2,mkNode($1,0),mkParent("ARGS",mkNode("NONE", 0)));}
		|	lhs '=' ID '('')' ';'				{$$ = mkNode("FUNCTION_CALL",1,(mkNode("ASS",3,$1,mkNode($3,0),mkParent("ARGS",mkNode("NONE", 0)))));}
		;
...
update:	lhs '=' expression 		{$$ = mkNode("ASS",2,mkNode("",1,$1),$3);}
		| 						{$$=NULL;}
		;
...
str_declaration: ID '[' integer_value ']' '=' STRING_VAL ';' {$$ = mkNode("STRING",3,mkNode($1,0),mkNode("[",2,$3,mkNode("]",0)), mkNode("ASS",1,mkNode($6,0)));}	
		| ID '[' integer_value ']' ';'						{$$ = mkNode("STRING",2,mkNode($1,0),mkNode("[",2,$3,mkNode("]",0)));}	
		| ID '[' integer_value ']' ','	str_declaration		{$$ = mkNode("STRING",3,mkNode($1,0),mkNode("[",2,$3,mkNode("]",0)),$6);}	
		;
...
variable_ass: ID '=' const_value ',' variable_ass	{$$ = mkNode("ASS",3,mkNode($1,1,$3), mkParent("",$5));}
		| ID '=' const_value					{$$ = mkNode("ASS",1,mkNode($1,1, $3));}
		| ID									{$$ = mkNode($1,0);}
		;
